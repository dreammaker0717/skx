            ...
            if(isset($_FIELDS) && count($_FIELDS)>0) {
                for($i=0;$i<count($_FIELDS);$i++) {                    
                    ...
                    if($part=="search" &&  $ad->sName=="st_lastcomment"  && $part!=="black") {
                        ...
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<input type=hidden  name=\"hcomment_"+row.ast_id+"\"  iden=\"s"+row.ast_id+"\" class=\"hcomment form-control form-control-sm\" value=\'"+dat+"\'><input type=text  name=\"comment_"+row.ast_id+"\" onkeyup=\'CommentTokeyUp(event,"+row["ast_id"]+")\' iden=\"s"+row.ast_id+"\" class=\"comment form-control form-control-sm\" value=\'"+dat+"\'>";  }},'."\r\n";
                    }
