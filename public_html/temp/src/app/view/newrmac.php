					...
					<form id="form1" autocomplete="off">
						<fieldset class="form-fieldset" style="width:100%; margin:0 auto;padding:2rem;">
							<div class="mb-3" style="width:500px; margin: 0 auto;">
								...
								<input style="max-width: 300px;display:inline-block" tabindex=5 type="text" class="form-control" id="scan_move" name="scan_move" style="text-align: center;" placeholder="Scan to see if exists in database" autocomplete="off"/>
...
$(function(){
	...
	function popNewProduct() {		
		$('#upd_npr_del').prop("checked",true);
		$('#rmac_products_new').modal("show");		
	}

	function createNewRmacProduct() {
		var pd = {
					action: "creatermacproduct",
					data: {						
						rmac_name: $('#upd_npr_name').val(),
						rmac_sku: $('#upd_npr_sku').val(),
						rmac_condition: $('#upd_npr_condition').val(),
						rmac_del: $('#upd_npr_del').prop("checked")
					}
				};

		$.ajax({
			url: "/newrmacajax/creatermacproduct",
			data: pd,
			type: 'POST',
			error: function(a) {
			...
			success: function(a) {
				...
				if (a.success) {
					successSound();
					$('#product').val(pd.data.rmac_name);
					$('#product-hidden').val(a.id);
					$('#product-sku-hidden').val(pd.data.rmac_sku);
					$('#product-hidden-table').val("RMAC");
					$('#createForm').get(0).reset();
					$('#rmac_products_new').modal("hide");		
				}
			}
		});
					...
					success: function(a) {
						...
						if (a.success) {
							...
							if(a.data.length>0) {
								...
								$('#fault').val(a.data[0]["ast_lastcomment"]);
...
<div id="rmac_products_new" class="modal" role="dialog" aria-modal="true" style="display: none;">
...
<div class="form-group mb-3">
                        ...
                        <input data-encrypt="" type="text" autocomplete="off" class="form-control" name="upd_npr_sku" id="upd_npr_sku" placeholder="Enter SKU"> 
                      </div>
<div class="form-group mb-3">
                        <label class="form-label" for="name">Name</label>
                        <input data-encrypt="" type="text" autocomplete="off" class="form-control" name="upd_npr_name" id="upd_npr_name" placeholder="Enter Name"> 
                      </div>
<div class="form-group mb-3">
                        <label class="form-label" for="name">Condition</label>
                        <input data-encrypt="" type="text" autocomplete="off" class="form-control" name="upd_npr_condition" id="upd_npr_condition" placeholder="Enter Condition"> 
                      </div>

<div class="form-group mt-4 mb-3">               
                    <div>
                      <label class="row">
                        <span class="col">Deleted</span>
                        <span class="col-auto">
                          <label class="form-check form-check-single form-switch">
                            <input class="form-check-input" type="checkbox" name="upd_npr_del" id="upd_npr_del">
                          </label>
                        </span>
                      </label>
                    </div>       
    ...
    <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary ms-auto" onClick="createNewRmacProduct()">Save</button>
