    ...
    try 
    {
        ...
        foreach($tokened as $key => $val) {
            ...
            foreach($val as $k=>$v) {
                ...
                $db->exec("INSERT INTO stock SET st_status=1, st_order=".$id.", st_product=".$pr_id.", st_servicetag='".$serial."', st_cost=".$cost.", st_date='".$currentDate."', st_addedby = '".$_SESSION["user_id"]."';");
...
if($action == "putorderitems") {
    ...
    {    
        ...
        if($id==0&&$csv!=null&&$csv!="") {
            ...
            $db->insert("orders",["or_date"=> date("Y-m-d", strtotime(str_replace("/","-",$data["date"])) ), "or_supplier"=>$data["supplier"], "or_state"=>"Completed"]);
            $id = $db->id();
        ...
        if($id==0) {
            $db->insert("orders",["or_date"=> date("Y-m-d", strtotime(str_replace("/","-",$data["date"])) ), "or_supplier"=>$data["supplier"], "or_state"=>"Awaiting"]);
            $id = $db->id();
            
        }
        else {
            $db->exec("update orders set or_date=:or_date, or_supplier=:or_supplier where or_id=:id",
                [
    ...
    $query=
        "SELECT *, 
            ...
            darkgreen as st_darkgreen, sold+gray as st_sold , black+stripped as st_black,  
             actionreq+actioncmp as st_action, brown as st_brown , CONCAT(  darkgreen+sold+gray, ' / ' , TRUNCATE( (((darkgreen+sold+gray)*100) / or_total_items),0),'%')  as st_fix_rate 
        FROM orders left join order_distribution on st_order = or_id, suppliers WHERE  $_SF
        order by $order_field 
        limit $start,$length";

//echo $query;
    $query_count = "select count(*) as c FROM orders left join order_distribution on st_order = or_id, suppliers WHERE  $_SFC ";
    $query_count2 = "select count(*) as c FROM orders left join order_distribution on st_order = or_id, suppliers WHERE  $_SF ";
