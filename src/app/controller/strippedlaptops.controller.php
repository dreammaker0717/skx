    ...
    try 
    {
        ...
        foreach($tokened as $key => $val) {
            ...
            foreach($val as $k=>$v) {
                ...
                $vat = 0;
                $netprice = 0;
                $orderQuery = "SELECT rfqorderid FROM dco_orders WHERE dor_id = " . $id;
                ...
                if($orderData[0]['rfqorderid'] != 0){
                    ...
                    foreach ($rfqopData as $entry) {
                        if ($entry['rfqop_prodtype'] == 4 && $entry['rfqop_product'] == $pr_id) {
                ...
                $db->exec("INSERT INTO dco_stock SET dst_status=1, dst_order=".$id.", dst_product=".$pr_id.", dst_servicetag='".$serial."', dst_date='".$currentDate."', dst_addedby = '".$_SESSION["user_id"]."', dst_cost=".$cost.", dst_vat_type='".$vattype."', dst_vat=".$vat.", dst_netprice=".$netprice.";");
...
if($action == "putorderitems") {
    ...
    {    
        ...
        if($id==0) {
            $db->insert("dco_orders",["dor_reference"=>$data["reference"],"dor_date"=> date("Y-m-d", strtotime(str_replace("/","-",$data["date"])) ), "dor_supplier"=>$data["supplier"], "dor_state"=>"Awaiting", "dor_type"=>"Standard"]);
            $id = $db->id();
