    ...
    try 
    {
        ...
        foreach($tokened as $key => $val) {
            ...
            foreach($val as $k=>$v) {
                ...
                $vat = 0;
                $netprice = 0;
                $orderQuery = "SELECT rfqorderid FROM acc_orders WHERE aor_id = " . $id;
                ...
                if($orderData[0]['rfqorderid'] != 0){
                    ...
                    foreach ($rfqopData as $entry) {
                        if ($entry['rfqop_prodtype'] == 3 && $entry['rfqop_product'] == $pr_id) {
                ...
                $db->exec("INSERT INTO acc_stock SET ast_status=1, ast_order=".$id.", ast_original_product=".$pr_id.",  ast_product=".$pr_id.", ast_servicetag='".$serial."',  ast_date='".$currentDate."', ast_addedby = '".$_SESSION["user_id"]."', ast_cost=".$cost.", ast_vat_type='".$vattype."', ast_vat=".$vat.", ast_netprice=".$netprice.";");
        ...
        $ordprods = $db->query("select aop_product from acc_orderprod WHERE aop_order = $id");

        foreach ($ordprods as $prod) {
            $orprod = $db->query("SELECT aop_quantity, aop_delivered from acc_orderprod where aop_order=".$id." and aop_product=".$prod['aop_product'])->fetchAll();
            if($orprod){
                if ($orprod[0]['aop_quantity'] != $orprod[0]['aop_delivered']) {
                    $isComplete = false;
                }
            } else {
                $isComplete = false;
        ...
        if($isComplete){
            $db->exec("update acc_orders set aor_state ='Completed' where aor_id=$id");
        }
...
if($action=="search") {
    ...
    $query=
        ...
        FROM acc_order_distribution left join suppliers on suppliers.sp_id = aor_supplier 
        WHERE  $_SF
        order by $order_field 
        limit $start,$length";


    $query_count = "select count(*) as c FROM acc_order_distribution WHERE  $_SFC ";
    $query_count2 = "select count(*) as c FROM acc_order_distribution WHERE  $_SF ";
