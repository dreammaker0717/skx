...
$_FIELDS[] = (object)array('sName' => 'st_magsku', 'title' => 'SKU', 'data' => 'st_magsku', 'type'=> 'string');
if( $part=="darkgreen") {
    $_FIELDS[] = (object)array('sName' => 'st_retail', 'title' => 'Price', 'data' => 'st_retail', 'type'=> 'string');
}
...
if($part=="sold" || $part=="darkgreen") {
    if($part=="darkgreen"){
        ...
        $_FIELDS[] = (object)array('sName' => 'st_soldprice', 'title' => 'Sold Price', 'data' => 'st_soldprice', 'type'=> 'string');
    } else if($part=="sold"){
        $_FIELDS[] = (object)array('sName' => 'st_soldprice', 'title' => 'Sold Price', 'data' => 'st_soldprice', 'type'=> 'string');
        $_FIELDS[] = (object)array('sName' => 'st_retail', 'title' => 'RRP', 'data' => 'st_retail', 'type'=> 'string');
        $_FIELDS[] = (object)array('sName' => 'st_soldto', 'title' => 'Sold To', 'data' => 'st_soldto', 'type'=> 'string');
        $_FIELDS[] = (object)array('sName' => 'st_soldcountry', 'title' => 'Country', 'data' => 'st_soldcountry', 'type'=> 'string');
        $_FIELDS[] = (object)array('sName' => 'st_soldmethod', 'title' => 'Type', 'data' => 'st_soldmethod', 'type'=> 'string');
                            ...
                            <thead>
                                ...
                                if(isset($_FIELDS) && count($_FIELDS)>0) {
                                    ...
                                    if(  ( $_STOCKCONFIG[$part]["comment_status"] === true && 
                                        !($part=="action" || $part=="darkgreen") && $ad->data=="st_lastcomment") || ($ad->data=="st_soldprice" && $part=="darkgreen") ) {  
        ...
        "columns" :[           
            ...
            if(isset($_FIELDS) && count($_FIELDS)>0) {
                for($i=0;$i<count($_FIELDS);$i++) {                    
                    ...
                    if(  ( $_STOCKCONFIG[$part]["comment_status"] === true && !($part=="action" || $part=="darkgreen") && $ad->data=="st_lastcomment") || ($ad->data=="st_soldprice" && $part=="darkgreen") ) {  
                    ...
                    else if($ad->data=="st_magsku") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){
                            return "<span onclick=\'MagSkuClick(event, this, "+row["st_id"]+")\' style=\"display: block; height: 2em;\">"+dat+"</span>";
                        }},'."\r\n";
                    }
                    ...
                    else if ($part=="darkgreen" && $ad->data=="st_retail") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){
                            var returnData = null;
                            if (row["st_magenabled"] == 1) {
                                if (dat == 0) {
                                    returnData = "<div style=\'border-left: 6px solid orange; padding-left: 6px; height: auto;\'><span style=\'width:140px; user-select: text;\' class=\'text-orange\'>"+dat+"</span></div>";
                                } else {
                                    returnData = "<div style=\'border-left: 6px solid green; padding-left: 6px; height: auto;\'><span style=\'width:140px; user-select: text;\' class=\'text-teal\'>"+dat+"</span></div>";
                                }
                            } else if (row["st_magenabled" == 2]) {
                                returnData = "<div style=\'border-left: 6px solid red; padding-left: 6px; height: auto;\'><span style=\'width:140px; user-select: text;\' class=\'text-pink\'>"+dat+"</span></div>";
                            } else {
                                returnData = "<div style=\'border-left: 6px solid magenta; padding-left: 6px; height: auto;\'><span style=\'width:140px; user-select: text;\' class=\'text-purple\'>"+dat+"</span></div>";
                            }
                            return returnData;
                        }},'."\r\n";
                    }
                    else if($part=="darkgreen" && $ad->data=="st_soldprice") {                                     
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<input type=text  name=\"soldprice_"+row.st_id+"\" onkeyup=\'SoldPricekeyUp(event,"+row["st_id"]+")\' iden=\"s"+row.st_id+"\" class=\"soldprice form-control form-control-sm\" value=\'"+dat+"\'>";  }},'."\r\n";
                    }
                    else                         
                        echo '{"data" : "'.$ad->data.'"},'."\r\n";
                }
...
function MagSkuClick(event, el, st_id) {
    var input = $("<input>", {val: $(el).text(), type: "text", class: "comment form-control form-control-sm", keyup: function(ev){
        console.log("Event here ",ev);
        if(ev.code=="Enter") {
            var pd = { action: "magsku", data: { 
                cm: ev.target.value            
             } };
            SendAction(pd,st_id);       
        }
    }, focusout: function(ev){
        console.log("Focus out here ",ev);
...
function SoldPricekeyUp(ev, st_id) {
    console.log("Event here ",ev);
    if(ev.code=="Enter") {
        var pd = { action: "sold_price", data: { 
            soldprice: ev.target.value            
         } };
        SendAction(pd,st_id);       
    }
}
