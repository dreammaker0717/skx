...
<?php
	...
	$_TITLE="Dell Components Stocks";


    if($BATCHED==0){

        $_FIELDS = [
            (object)array('sName' => 'dst_id', 'title' => 'ID', 'data' => 'dst_id', 'type'=>'number' ),    
            (object)array('sName' => 'dp_image', 'title' => 'Image', 'data' => 'dp_image', 'type'=> 'string'),
        ...
        $_FIELDS[] = (object)array('sName' => 'dp_condition', 'title' => 'Condition', 'data' => 'dp_condition', 'type'=> 'string');        
		 

        $_FIELDS[] = (object)array('sName' => 'dst_servicetag', 'title' => 'Service Tag', 'data' => 'dst_servicetag', 'type'=> 'string');    
        ...
        $_FIELDS[] = (object)array('sName' => 'dst_lastcomment', 'title' => 'Last Comment', 'data' => 'dst_lastcomment', 'type'=> 'string');

    }
    else {
        $_FIELDS = [
            (object)array('sName' => 'dp_id', 'title' => 'ID', 'data' => 'dp_id', 'type'=>'number' ),                
            (object)array('sName' => 'dp_sku', 'title' => 'SKU', 'data' => 'dp_sku', 'type'=> 'string'),
            (object)array('sName' => 'dp_name', 'title' => 'Name', 'data' => 'dp_name', 'type'=> 'string'),      
        ];
        //$_FIELDS[] = (object)array('sName' => 'ct_name', 'title' => 'Category', 'data' => 'ct_name', 'type'=> 'string');    
        ...
        $_FIELDS[] = (object)array('sName' => 'dp_listed', 'title' => 'List Status', 'data' => 'dp_listed', 'type'=> 'string');                
        

        $_FIELDS[] = (object)array('sName' => 'counter', 'title' => 'QTY', 'data' => 'counter', 'type'=> 'string');

        if($part=="lightgreen" || $part=="green" || $part=="darkgreen") {
            $_FIELDS[] = (object)array('sName' => 'dp_magqty', 'title' => 'MAGENTO QTY', 'data' => 'dp_magqty', 'type'=> 'string');
        }
    }
?>
<style>
                    ...
                    <div class="table-responsive" style="padding:5px;margin:5px;">
                        ...
                        <div class="col-sm-2">
								<select onchange="window.ETable.fnDraw()" class="form-control" id="category" name="category">
									...
									<?php
                                       ...
                                       $_SF = "dst_status=43"; 
                                        ...
                                        $cxx = M::db()->exec("select dp_category from dco_stock join dell_part on dco_stock.dst_product = dell_part.dp_id where dco_stock.".$_SF." group by dp_category")->fetchAll();
                                        $cx = array();
                            ...
                            <div class="col-sm-2">
                                ...
                                <a class="btn btn-indigo" style="width:2.5rem;" href="/componentstocks/<?php echo $part; ?>"  data-bs-toggle="tooltip" data-bs-placement="top" title="List View">
...
<a class="btn  btn-purple" style="width:2.5rem;" href="/componentstocks_batch/<?php echo $part; ?>"  data-bs-toggle="tooltip" data-bs-placement="top" title="Batch View">
							...
							<thead>
								...
								<!--	<th class='th_checkbox'>A</th> -->
									<?php
										...
										if(  $ad->data=="dst_lastcomment" && $part=="black" /*( $_ASTOCKCONFIG[$part]["comment_status"] === true && 
										!($part=="action" || $part=="darkgreen") && $ad->data=="dst_lastcomment") || ($ad->data=="ast_retail" && $part=="darkgreen") */) {  
										echo "<th class='th_dst_status'>Status1</th>";
										}
										if($part=="search" && $ad->data=="dst_lastcomment" && $part!=="sold") {
										echo "<th class='th_dst_status'>Status2</th>";
										}
...
function condition_check(r){
    ...
    if(part == "lightgreen" && r.dp_condition != "Refurbished (Grade B)")
        return "<span title='Condition and SKU mismatch!'>⚠️</span>";
    if(part == "green" && r.dp_condition != "Refurbished")
        return "<span title='Condition and SKU mismatch!'>⚠️</span>";
    if(part == "darkgreen" && r.dp_condition != "New" && r.dp_condition!='New - Open Box' && r.dp_condition !='New - Brown Box')
        return "<span title='Condition and SKU mismatch!'>⚠️</span>";
...
$(function() {
    ...
    window.ETable = $('#dataList').dataTable({
        ...
        "ajax":{
            url:"/componentstocksajax/<?php echo $part; ?>",
            type:"POST",
  ...
  <!--              return "<input  class='form-check-input dts' type=checkbox iden='s"+row.ast_id+"' />"; } }, -->
            ...
            if(isset($_FIELDS) && count($_FIELDS)>0) {
                for($i=0;$i<count($_FIELDS);$i++) {                    
                    ...
                    if($part=="search" &&  $ad->sName=="dst_lastcomment"  && $part!=="black") {
                        echo '{"data" : "'.$ad->data.'", "render":function(data,type,row){ return statusText(row.st_status); } },'."\r\n";
                    }
                    

                    if($ad->data=="dst_id")                      
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return condition_check(row)+ "<a class=stid href=\'/componentstock/"+dat+"?s='.$part.'\'>"+dat+"</a>";}},'."\r\n";

                    else if($ad->data=="dp_image") 
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<a href=\'javascript:PrintBoxLabels("+row.dst_id+")\'><img src=\'https://img.icons8.com/material-outlined/24/000000/print.png\'></a> <a href=\'https://www.ndc.co.uk/pub/media/catalog/product"+dat+"\' onClick=\'return linkModelImage(this,event)\' target=_blank><img src=\'https://img.icons8.com/material-outlined/24/000000/image.png\'/></i></a>"  }},'."\r\n";                        
                    else if($ad->data=="dst_date" || $ad->data=="st_actionreq_date" || $ad->data=="st_actioncmp_date") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat){ return timeAgo(dat); }},'."\r\n";
                    }
                    else if( ($ad->data=="dst_status" || $ad->data =="st_status_action")) {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat){ return statusText(dat); }},'."\r\n";
                    }
                    else if($ad->data=="dp_sku") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<div data-id="+row.dst_id+" >"+ (dat==null ? "<a href=\"javascript:void(0)\" onclick=\"assign_new(this)\">No Product Assigned – Assign</a>" : dat+" <a onclick=\'change_sku(\""+row.dst_servicetag+"\",\""+row.dp_id+"\")\' href=#>📇</a>") + "</div>"; }},'."\r\n";
                    }
                    else if($ad->data=="dst_state") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<div data-id="+row.dst_id+" >"+dropDownState(dat)+"</div>"; }},'."\r\n";
                    }
                    else if($ad->data=="dp_listed") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<div data-id="+row.dp_id+" >"+dropDownListed(dat)+"</div>"; }},'."\r\n";
                    }
                    else if($ad->data=="dst_record") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<div data-id="+row.dst_id+" >"+dropDownRecOrd(dat)+"</div>"; }},'."\r\n";
                    }                    
                    else  if( ( $part=="darkgreen") &&  $ad->sName=="st_lastcomment")
                            continue;
                    else if($part!=="search"  && $ad->data=="dst_lastcomment") {                                     
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return type=="export" ? dat :  "<input type=hidden  name=\"hcomment_"+row.dst_id+"\"  iden=\"s"+row.dst_id+"\" class=\"hcomment form-control form-control-sm\" value=\'"+dat+"\'><input type=text  name=\"comment_"+row.dst_id+"\" onkeyup=\'CommentTokeyUp(event,"+row["dst_id"]+")\' iden=\"s"+row.dst_id+"\" class=\"comment form-control form-control-sm\" value=\'"+dat+"\'>";  }},'."\r\n";
                    }
            ...
            ?>
                ...
                { "data":null, sortable:false, searchable:false, "render":function(data, type, row){ return "<button class='btn btn-sm btn-success' onClick='actionCompleted("+row["dst_id"]+")'>Complete</button>"; } },                                
            <?php 
                }
            ?>
            ...
            <?php
                ...
                ?>
                { "data":null, sortable:false, searchable:false, "render":function(data, type, row){ return "<button class='btn btn-sm btn-warning' onClick='actionCancelled("+row["dst_id"]+")'>Cancel</button>"; } },                
            <?php 
            }
            ?>
...
function SendAction(pd,ast_id) {
    $.ajax({ 
            url:"/componentstocksajax/"+ast_id,
            data:pd,
            type:'POST', 
            success:function(a) {
            a = JSON.parse(a);                                   
...
function PrintBoxLabels(d) {
    ...
    if(part=="orange")
        PrintSection("dp-print-orange-box-labels",d);
    else if(part=="red")
        PrintSection("dp-print-red-box-labels",d);
    else if(part=="purple")
        PrintSection("dp-print-purple-box-labels",d);
    else 
        PrintSection("dp-print-box-labels",d);
}
function PrintSection(type, id) {
    ...
    if(stock_items.length>0){
        ...
        $('#stock_items_form').attr("action","../componentstocksprintajax/"+type);
