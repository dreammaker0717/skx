...
<?php
	...
	$_CHANGE_STATUSES = "";
	for($i=0;$i<count($_NPSTOCKCONFIG[$part]["status"]);$i++) {
		$data = $_NPSTOCKCONFIG[$part]["status"][$i];    
		
		$_CHANGE_STATUSES.="<option value='".$data."' style='color:".$_NPSTATUSES[$data]["Color"]."'>".$_NPSTATUSES[$data]["Name"]."</option>";
	}
	function MakeStatusText($t){
		if(!$t) return "";
		global $_NPSTATUSES;
		foreach($_NPSTATUSES as $key=>$val) {
			if($key == $t) {
				return "<span>"+$val["Name"]+"</span>";
			}
		}
	}
	$_TITLE="New Product Stocks";


    if($BATCHED==0){

	$_FIELDS = [
		(object)array('sName' => 'nst_id', 'title' => 'ID', 'data' => 'nst_id', 'type'=>'number' ),    
		(object)array('sName' => 'npr_image', 'title' => 'Image', 'data' => 'npr_image', 'type'=> 'string'),
	...
	$_FIELDS[] = (object)array('sName' => 'ct_name', 'title' => 'Category', 'data' => 'ct_name', 'type'=> 'string');    
        ...
        if(!($part=="red"||$part=="rts")) {

	$_FIELDS[] = (object)array('sName' => 'npr_condition', 'title' => 'Condition', 'data' => 'npr_condition', 'type'=> 'string');        
		 
		 
   }

	$_FIELDS[] = (object)array('sName' => 'nst_servicetag', 'title' => 'Service Tag', 'data' => 'nst_servicetag', 'type'=> 'string');    
	...
	//$_FIELDS[] = (object)array('sName' => 'apr_box_subtitle', 'title' => 'Box Subtitle', 'data' => 'apr_box_subtitle', 'type'=> 'string');        
        ...
        if($part=="red"||$part=="rts") {
            $_FIELDS[] = (object)array('sName' => 'nor_date', 'title' => 'Order Date', 'data' => 'nor_date', 'type'=> 'string');        
            $_FIELDS[] = (object)array('sName' => 'sp_name', 'title' => 'Supplier', 'data' => 'sp_name', 'type'=> 'string');        
            $_FIELDS[] = (object)array('sName' => 'nor_reference', 'title' => 'Order Reference', 'data' => 'nor_reference', 'type'=> 'string');        
        }
      $_FIELDS[] = (object)array('sName' => 'nst_lastcomment', 'title' => 'Last Comment', 'data' => 'nst_lastcomment', 'type'=> 'string');
        ...
        $_FIELDS = [
            (object)array('sName' => 'npr_id', 'title' => 'ID', 'data' => 'npr_id', 'type'=>'number' ),                
            (object)array('sName' => 'npr_sku', 'title' => 'SKU', 'data' => 'npr_sku', 'type'=> 'string'),
            (object)array('sName' => 'npr_name', 'title' => 'Name', 'data' => 'npr_name', 'type'=> 'string'),      
        ];
        $_FIELDS[] = (object)array('sName' => 'ct_name', 'title' => 'Category', 'data' => 'ct_name', 'type'=> 'string');    
        ...
        $_FIELDS[] = (object)array('sName' => 'npr_listed', 'title' => 'List Status', 'data' => 'npr_listed', 'type'=> 'string');                
		

        $_FIELDS[] = (object)array('sName' => 'counter', 'title' => 'QTY', 'data' => 'counter', 'type'=> 'string');

        if($part=="darkgreen") {
            $_FIELDS[] = (object)array('sName' => 'npr_magqty', 'title' => 'MAGENTO QTY', 'data' => 'npr_magqty', 'type'=> 'string');
        }
    }
?>
<style>
                        ...
                        <div class="row align-items-center">
                            <div class="col">
                                <div class="page-pretitle"> 
                                    <h2 class="btn btn-square <?php echo $status_color;?>" style="width:100%;"><?php echo $part ." - ". $_NPSTOCKCONFIG[$part]["title"]; ?></h2>
                                </div>
                            </div>                           
                        </div>
							...
							<div class="col-sm-2">
								<select onchange="window.ETable.fnDraw()" class="form-control" id="category" name="category">
									...
									<?php
                                        ...
                                        //$cx = M::db()->select("nwp_products","npr_category",['GROUP'=>'npr_category']);     
...
<a class="btn btn-indigo" style="width:2.5rem;" href="/newitemstocks/<?php echo $part; ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="List View">
...
<a class="btn  btn-purple" style="width:2.5rem;" href="/newitemstocks_batch/<?php echo $part; ?>" data-bs-toggle="tooltip" data-bs-placement="top" title="Batch View">
							...
							<thead>
								...
								<!--	<th class='th_checkbox'>A</th> -->
									<?php
										...
										if(  $ad->data=="nst_lastcomment" && $part=="black" /*( $_NPSTOCKCONFIG[$part]["comment_status"] === true && 
										!($part=="action" || $part=="darkgreen") && $ad->data=="nst_lastcomment") || ($ad->data=="ast_retail" && $part=="darkgreen") */) {  
										echo "<th class='th_nst_status'>Status1</th>";
										}
										if($part=="search" && $ad->data=="nst_lastcomment" && $part!=="sold") {
										echo "<th class='th_nst_status'>Status2</th>";
										}
...
$(function() {
    ...
    window.ETable = $('#dataList').dataTable({
        ...
        "ajax":{
            url:"/newitemstocksajax/<?php echo $part; ?>",
            type:"POST",
  ...
  <!--              return "<input  class='form-check-input dts' type=checkbox iden='s"+row.ast_id+"' />"; } }, -->
            ...
            if(isset($_FIELDS) && count($_FIELDS)>0) {
                for($i=0;$i<count($_FIELDS);$i++) {                    
                    ...
                    else if($ad->data=="npr_sku") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<div data-id="+row.nst_id+" >"+ (dat==null ? "<a href=\"javascript:void(0)\" onclick=\"assign_new(this)\">No Product Assigned – Assign</a>" : dat+" <a onclick=\'change_sku(\""+row.nst_servicetag+"\",\""+row.npr_id+"\")\' href=#>📇</a>") + "</div>"; }},'."\r\n";
                    }
                    else if($ad->data=="st_state") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<div data-id="+row.nst_id+" >"+dropDownState(dat)+"</div>"; }},'."\r\n";
                    }
                    else if($ad->data=="npr_listed") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<div data-id="+row.npr_id+" >"+dropDownListed(dat)+"</div>"; }},'."\r\n";
                    }
                    else if($ad->data=="st_record") {
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<div data-id="+row.nst_id+" >"+dropDownRecOrd(dat)+"</div>"; }},'."\r\n";
                    }                    
                    else  if( ( $part=="darkgreen") &&  $ad->sName=="st_lastcomment")
                            continue;
                    else if($part!=="search"  && $ad->data=="nst_lastcomment") {                                     
                        echo '{"data" : "'.$ad->data.'", "render":function(dat,type,row){ return "<input type=hidden  name=\"hcomment_"+row.nst_id+"\"  iden=\"s"+row.nst_id+"\" class=\"hcomment form-control form-control-sm\" value=\'"+dat+"\'><input type=text  name=\"comment_"+row.nst_id+"\" onkeyup=\'CommentTokeyUp(event,"+row["nst_id"]+")\' iden=\"s"+row.nst_id+"\" class=\"comment form-control form-control-sm\" value=\'"+dat+"\'>";  }},'."\r\n";
                    }
...
function PrintSection(type, id) {
    ...
    if(stock_items.length>0){
        ...
        $('#stock_items_form').attr("action","../newitemstocksprintajax/"+type);
      ...
      <div class="modal-body">
        <form>
          <div class="form-group">
            ...
            <?php
                $spp = $db->query("SELECT  npr_id, npr_sku, npr_name FROM nwp_products where npr_del=0 order by npr_sku, npr_name asc")->fetchAll();
                foreach($spp as $k=>$v) {    
                    echo "<option  value=".$v["npr_id"].">".$v["npr_sku"]." - ".$v["npr_name"]."</option>";
                }
            ?>            
			</select>					 
          </div>          
